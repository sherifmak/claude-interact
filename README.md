...
<select data-loop="Font">
  <option>Inter</option>
  <option>Söhne</option>
</select>
<textarea data-loop="Hero copy">Build software at the speed of thought.</textarea>
...
Include claude-loop so I can send my answers back to you:
  <link rel="stylesheet" href="loop.css">
  <script src="loop.js" defer></script>
Mark each control with data-loop="Friendly label", lists with
data-loop-order, and decisions with data-loop-decision.
...
| --- | --- | --- |
| `data-loop="Label"` | any `input`, `select`, `textarea`, contenteditable | Captures the field's value under `Label`. Checkboxes report `yes`/`no`; selects report the option text; radios report the checked option. |
| `data-loop-on` / `data-loop-off` | a `data-loop` checkbox | Overrides the `yes`/`no` text for checked / unchecked (e.g. `data-loop-on="enabled"`). |
| `data-loop-order="Label"` | a list container (`ol`/`ul`/`div`) | Emits an ordered, top-to-bottom list of its items under `Label`. |
| `data-loop-item` | each item inside a `data-loop-order` list | Marks a list entry. |
| `data-loop-text` | a node inside an item | The text used for that item (falls back to the item's own text). |
| `data-loop-toggle` | a checkbox inside an item | Unchecking it marks the item `— REMOVED` in the receipt. |
| `data-loop-decision="Label"` | a group container | A single-choice decision. Clicking a choice inside it activates that one. |
| `data-loop-choice="Value"` | a `button` inside a decision group | One option. The active choice's value is reported under the group's `Label`. |
| `data-loop-title="…"` | any element | Sets the receipt title (otherwise the cleaned-up `document.title` is used). |

### JavaScript API

The global `window.Loop` lets you drive the receipt from code. Methods that mutate config or values return `Loop`, so they chain.

| Call | Effect |
| --- | --- |
| `Loop.config({ title, button, intro, outro })` | Override the receipt title, the launcher button label, and the intro / outro lines. |
| `Loop.value(label, value)` | Register (or update) a field. Overrides a matching `data-loop` label, else appends. |
| `Loop.remove(label)` | Drop a JS-registered field by label. |
| `Loop.collect()` | Return `{ title, fields }` — the structured data behind the receipt. |
| `Loop.format()` | Return the receipt as the exact pasteable string. |
| `Loop.open()` | Open the receipt modal (and auto-copy). |

```js
Loop.config({ title: 'Trip planner', button: 'Send to Claude' });
...
On load, `loop.js` injects a floating launcher and a receipt modal, then wires up any decision groups. When you open the receipt it scans every `data-loop`, `data-loop-order`, and `data-loop-decision` node in DOM order so the receipt reads top-to-bottom exactly like the page, merges in anything registered via `Loop.value`, formats it into a clean text block, and copies it to your clipboard (with a legacy `execCommand` fallback). Paste it back into the chat and the loop is closed.

## Project structure

```text
claude-loop/
├── loop.js          # the drop-in (zero dependencies)
├── loop.css         # shared styles for the launcher + receipt
